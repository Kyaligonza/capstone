...
class MyapphsbkTestCase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        # self.producer_auth_header = {'Authorization': 'Bearer {}'.format(self.producer_token)}
        self.producer_auth_header = {'Authorization':'Bearer {}'.format(bearer_tokens['executive_producer'])}
        self.director_auth_header = {'Authorization':'Bearer {}'.format(bearer_tokens['casting_director'])}
        self.assistant_auth_header = {'Authorization':'Bearer {}'.format(bearer_tokens['casting_assistant'])}
    ...
    def test_delete_actor(self):
        res = self.client().delete('/actors/12',headers = self.assistant_auth_header)
        data = json.loads(res.data)
